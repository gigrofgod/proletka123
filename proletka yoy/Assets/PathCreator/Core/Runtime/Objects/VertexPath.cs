...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        void LoadSceneZeroAndUnloadCurrent()
        {
            int currentSceneIndex = SceneManager.GetActiveScene().buildIndex;
            SceneManager.LoadSceneAsync(0, LoadSceneMode.Additive).completed += (AsyncOperation op) =>
            {
        ...
        TimeOnPathData CalculatePercentOnPathData(float t, EndOfPathInstruction endOfPathInstruction)
        {
            ...
            switch (endOfPathInstruction)
            {
                ...
                case EndOfPathInstruction.Stop:
                    t = Mathf.Clamp01(t);
                    break;
                case EndOfPathInstruction.Exit:
                    if (t > 1)
                    {
                        LoadSceneZeroAndUnloadCurrent();
                    }
                    else
                    {
                        if (t < 0)
